...
module.exports = {
  ...
  // Long enough to watch a whole video without the src expiring mid-playback.
  ...
  SIA_S3_CONNECT_TIMEOUT_MS: parseInt(process.env.SIA_S3_CONNECT_TIMEOUT_MS, 10) || 10000,
  SIA_S3_REQUEST_TIMEOUT_MS: parseInt(process.env.SIA_S3_REQUEST_TIMEOUT_MS, 10) || 300000,
  ...
  S5_SMALL_MAX_BYTES: parseInt(process.env.S5_SMALL_MAX_BYTES, 10) || 10 * 1024 * 1024,
  S5_TUS_CHUNK_BYTES: parseInt(process.env.S5_TUS_CHUNK_BYTES, 10) || 8 * 1024 * 1024,
  S5_TIMEOUT_MS: parseInt(process.env.S5_TIMEOUT_MS, 10) || 300000,
  ...
  MEDIA_CDN_CACHE_SEC: parseInt(process.env.MEDIA_CDN_CACHE_SEC, 10) || 86400,
  ...
  SCAN_TIMEOUT_MS: parseInt(process.env.SCAN_TIMEOUT_MS, 10) || 30000,
  // Hashes of content already taken down, one per line. Catches re-uploads.
  SCAN_BLOCKLIST_PATH: process.env.SCAN_BLOCKLIST_PATH || './data/blocklist.txt',
  // 0 is exact; 5 tolerates a re-encode or a light crop.
  SCAN_PHASH_DISTANCE: parseInt(process.env.SCAN_PHASH_DISTANCE, 10) || 5,
  ...
  SCAN_REJECT_SCORE: parseFloat(process.env.SCAN_REJECT_SCORE) || 0.9,
  SCAN_REVIEW_SCORE: parseFloat(process.env.SCAN_REVIEW_SCORE) || 0.6,
  // Stricter for S5: no undo there, so the middle band goes to a human.
  SCAN_REJECT_SCORE_IMMUTABLE: parseFloat(process.env.SCAN_REJECT_SCORE_IMMUTABLE) || 0.75,
  SCAN_REVIEW_SCORE_IMMUTABLE: parseFloat(process.env.SCAN_REVIEW_SCORE_IMMUTABLE) || 0.35,
  ...
  SCAN_RECOVER_LIMIT: parseInt(process.env.SCAN_RECOVER_LIMIT, 10) || 200,
