...
if (!file) {
  console.error('usage: node test/upload-test.js <video-file> [--interrupt]');
  process.exit(1);
}
...
const upload = new tus.Upload(stream, {
  ...
  metadata: { filename: path.basename(file), filetype: 'video/mp4' },
  ...
  async onSuccess() {
    ...
    console.log(`\nuploaded, id=${id} — polling status...`);
    for (;;) {
      const res = await fetch(`${ENDPOINT}/videos/${id}/status`, {
        headers: { 'x-upload-key': KEY },
      });
